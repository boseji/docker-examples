...
# ~ Directo Command without any Target since the File only has one
#docker build -t builder -f builder.Dockerfile .
# ~ Final Section command in the Combo File to stop after 'builder' Target
# ~   Also note the addtional Tag assigned to the Builder
...
echo
# ~ Inistial Command with no target since only one exists
